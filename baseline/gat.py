...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        for i in range(args.layer):
            head = 1 if i == args.layer - 1 else 8
            dim_in = dataset.num_features if i == 0 else args.hidden
            dim_out = dataset.num_classes if i == args.layer - 1 else args.hidden // head
            concat = False if i == args.layer - 1 else True
            self.convs.append(GATConv(dim_in, dim_out, head, concat=concat))
